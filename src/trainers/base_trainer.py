...
class BaseTrainer:
            ...
            if self.monitor_mode != 'off' : # Then there is a metric to monitor
                if self.monitor_metric in log: # Then we have measured it in this epoch
                    ...
                    if self._is_improvement(current_value):
                        ...
                        best_model_path = os.path.join(self.checkpoint_dir, 'best_val_model.pth')
                        self.save_model(path=best_model_path)
                        self.logger.info(f"New best model found at epoch {self.current_epoch}, saving model.")
                        self.not_improved_count = 0
                    else:
    ...
    def _is_improvement(self, current_value):
      if self.monitor_mode == 'min':
        return current_value < self.monitor_best
      else:
        return current_value > self.monitor_best
